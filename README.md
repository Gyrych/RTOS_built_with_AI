...
- **开发板**: 星火一号开发板
- **LED指示**: 
  - 绿色LED: GPIOF Pin 11 (PF11)
  - 红色LED: GPIOF Pin 12 (PF12)
- **串口通信**: UART1 - PA9(TX), PA10(RX), 115200波特率
...
1. **绿色LED闪烁任务** (优先级1) - PF11引脚，100ms周期闪烁
   - 亮50ms，灭50ms，总周期100ms
   - 展示高频率任务调度

2. **红色LED闪烁任务** (优先级2) - PF12引脚，500ms周期闪烁
   - 亮250ms，灭250ms，总周期500ms
   - 展示中等频率任务调度

3. **串口打印任务** (优先级3) - UART1输出"Hellow rtos!"
   - 每1000ms输出一次消息
   - 展示串口通信和printf重定向功能
